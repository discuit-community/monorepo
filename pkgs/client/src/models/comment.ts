import type { Comment, CommentVoteRequest, List, ListResponses } from "@discuit-community/types";
...
export class CommentModel extends Model {
	...
	async update(body: string): Promise<Result<CommentModel>> {
		const endpoint = `${this.client.urls.api.comments(this.data.postPublicId)}/${this.data.id}`;
	...
	async save(listId: string): Promise<Result<CommentModel>> {
		...
		const [_data, error] = await this.apiCall<ListResponses.GetListItems>("POST", endpoint, {
			targetType: "comment",
	...
	async unsave(listId: string): Promise<Result<CommentModel>> {
		...
		const [_data, error] = await this.apiCall<ListResponses.GetListItems>("DELETE", endpoint, {
			targetType: "comment",
			targetId: this.data.id,
		});
